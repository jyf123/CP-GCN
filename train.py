...
parser.add_argument('--e_weight', type=float, default=0.1, help='the weight of explainer.')
parser.add_argument('--explainer', type=str, default="explanation/semeval_top20/model-300epoch-best.ckpt", help='the explainer model.')
parser.add_argument('--d_weight', type=float, default=0.9, help='the weight of dep.')
...
for epoch in range(1, opt['num_epoch']):
    ...
    print("epoch {}: train_loss = {:.6f}".format(epoch,\
        train_loss))
    # file_logger.log("{}\t{:.6f}\t{:.6f}\t{:.6f}\t{}\t{}\t".format(epoch, train_loss, opt['e_weight'], opt['d_weight'], opt['e_weight'].grad, opt['d_weight'].grad))
