...
cg_dict = torch.load('%s/train-graphs-full.ckpt'%(args.dataset))
...
class GraphSampler(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.graph_data = [load_graph(graph_idx) for graph_idx in tqdm(graph_idxs)]
...
def load_graph(graph_idx):
    ...
    sub_adj = torch.from_numpy(np.int64(data['adj00']>0)).float()
    ...
    if args.graph_labeling:
        ...
        graph_label_onehot = label_onehot[graph_label] #100*100
        graph_label_onehot = graph_label_onehot.to(device)
    ...
    adj_label = sub_loss_diff + np.eye(sub_loss_diff.shape[0])#加上自环
    ...
    return {
        "graph_idx": graph_idx,
        "sub_adj": sub_adj.to("cpu"),
        "adj_norm": adj_norm.to("cpu").float(),
        "sub_feat": sub_feat.to("cpu").float(),
        "sub_label": sub_label.to("cpu").float(),
        "sub_loss_diff": sub_loss_diff.to("cpu").float(),
        "adj_label": adj_label.to("cpu").float(),
        "n_nodes": torch.from_numpy(np.array(n_nodes)).to("cpu"),
...
def main():
    ...
    def eval_graph(dataset):
        ...
        graph_idxs = []
        for data in dataset:
            sub_adj = data['sub_adj'].to(device)
            adj_norm = data['adj_norm'].to(device)
            sub_feat = data['sub_feat'].to(device)
            sub_loss_diff = data['sub_loss_diff'].to(device)
    ...
    def mse(x,mu,logvar,data):
        # return torch.nn.MSELoss(x.view(x.shape[0], -1), data['adj_label'].view(x.shape[0], -1),reduce=True, size_average=True)
